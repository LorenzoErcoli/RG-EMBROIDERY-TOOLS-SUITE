...
// cucire il tool vero.
//
//   npx esbuild apps/pettine/scripts/provini-dst.ts --bundle --format=esm --platform=node \
//     --alias:@rg/core=./packages/core/src/index.ts --outfile=apps/pettine/scripts/provini-dst.mjs
//   node apps/pettine/scripts/provini-dst.mjs
...
writeFileSync('apps/pettine/scripts/out/provini.svg', svg, 'utf8');
...
writeFileSync('apps/pettine/scripts/out/provini.dst', dst);

console.log(`-> apps/pettine/scripts/out/provini.svg  (${TW} × ${TH} mm)`);
console.log(`-> apps/pettine/scripts/out/provini.dst  (${(dst.length / 1024).toFixed(0)} kB)`);
