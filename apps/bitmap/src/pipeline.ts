...
import { stitchSteps, analyzeBitmap, type BitmapParams, type StitchColor, type StitchProgress } from './engine';
...
export interface BitmapPreviewResult {
  ...
  totalPixels: number;
  /** Punti che l'anteprima DOVREBBE disegnare: sono quelli preparati, cioè quelli che verranno cuciti. */
  previewPoints: number;
  /** Punti disegnati davvero: se è minore di previewPoints il disegno è diradato (solo visivo). */
  drawnPoints: number;
}

/**
 * Tetto di DISEGNO dell'anteprima. È un limite di sola visualizzazione, mai del ricamo.
 * Alzato da 120.000 a 300.000 perché il conteggio dei punti non dipende dai pixel ma solo dalla
 * misura in mm e dalla distanza fra i punti (larghezza/distanza x altezza/distanza): a 1,2mm il
 * vecchio tetto scattava gia' intorno ai 415mm di lato, e un'immagine grande mostrava buchi
 * regolari pur essendo ricamata tutta. Chi supera anche questo se lo sente DIRE dal pannello
 * (avviso in tool.ts) invece di vedere buchi che non esistono.
 */
export const PREVIEW_MAX_DOTS = 300000;
 ...
 * Ritorna anche QUANTI punti ha disegnato: se sono meno dei preparati, il pannello lo dichiara.
 */
function buildPreviewSvg(colors: { color: string; points: { x: number; y: number }[] }[], bounds: Bounds, dotMm: number, mmPerPx: number, maxTotal = PREVIEW_MAX_DOTS): { svg: string; drawn: number } {
  const m = 4;
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${r3(w)}mm" height="${r3(h)}mm" viewBox="${vb}">\n${paths.join('\n')}\n</svg>\n`;
  return { svg, drawn };
}
...
export function runBitmapPreview(
  ...
  const dotMm = 0.6;
  const { svg, drawn } = buildPreviewSvg(a.colors, bounds, dotMm, mmPerPx);
  ...
  const previewPoints = a.colors.reduce((s, c) => s + c.points.length, 0);
  return { svg, bounds, colors, selectedPixels: a.selectedPixels, totalPixels: a.totalPixels, previewPoints, drawnPoints: drawn };
}
 ...
 */
export function* bitmapPipelineSteps(
  rgba: Uint8ClampedArray | number[],
  width: number,
  height: number,
  params: BitmapParams,
  mmPerPx: number,
  onlyColor?: string,
): Generator<StitchProgress, BitmapPipelineResult, void> {
  const result = yield* stitchSteps(rgba, width, height, params, mmPerPx, onlyColor);
...
/** La pipeline in un colpo solo, senza avanzamento: drena `bitmapPipelineSteps`. */
export function runBitmapPipeline(
  ...
  const g = bitmapPipelineSteps(rgba, width, height, params, mmPerPx, onlyColor);
  let r = g.next();
