...
//
// La geometria non sta piu' qui: sta in `apps/pettine_v2/src/motore.ts`, che e' lo stesso codice che
...
//
//   npx esbuild apps/pettine_v2/scripts/livelli.ts --bundle --format=esm --platform=node \
//     --alias:@rg/core=./packages/core/src/index.ts \
//     --alias:@rg/pattern-grammar=./packages/pattern-grammar/src/index.ts \
//     --outfile=apps/pettine_v2/scripts/livelli.mjs
//   node --max-old-space-size=4096 apps/pettine_v2/scripts/livelli.mjs <file.svg> [basi] [sormonto]
...
writeFileSync('apps/pettine_v2/scripts/out/verifica-livelli.svg', esito.svgVerifica, 'utf8');
console.log('-> apps/pettine_v2/scripts/out/verifica-livelli.svg');
if (DENTI) {
  writeFileSync(`apps/pettine_v2/scripts/out/${nome}.svg`, esito.svg, 'utf8');
  console.log(`-> apps/pettine_v2/scripts/out/${nome}.svg`);
  if (esito.dst) {
    writeFileSync(`apps/pettine_v2/scripts/out/${nome}.dst`, esito.dst);
    console.log(`-> apps/pettine_v2/scripts/out/${nome}.dst (${esito.dst.length} byte)`);
  }
}
