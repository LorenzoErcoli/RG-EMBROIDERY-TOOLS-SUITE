...
//
//   npx esbuild apps/pettine_v2/scripts/armonia.ts --bundle --format=esm --platform=node \
//     --alias:@rg/core=./packages/core/src/index.ts --outfile=apps/pettine_v2/scripts/armonia.mjs
//   node --max-old-space-size=4096 apps/pettine_v2/scripts/armonia.mjs <file.svg> [basi] [sormonto]
...
writeFileSync('apps/pettine_v2/scripts/out/verifica-armonia.svg', `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -18 ${WM.toFixed(1)} ${(HM + 18).toFixed(1)}" width="${WM.toFixed(1)}mm" height="${(HM + 18).toFixed(1)}mm">
<rect x="0" y="-18" width="${WM.toFixed(1)}" height="${(HM + 18).toFixed(1)}" fill="#faf9f7"/>
<g transform="translate(0,-18)">${legenda}${nota}</g>
${pezzi.join('\n')}
</svg>`, 'utf8');
console.log('-> apps/pettine_v2/scripts/out/verifica-armonia.svg');
