...
//
//   npx esbuild apps/pettine_v2/scripts/blocchi.ts --bundle --format=esm --platform=node \
//     --alias:@rg/core=./packages/core/src/index.ts --outfile=apps/pettine_v2/scripts/blocchi.mjs
//   node --max-old-space-size=6144 apps/pettine_v2/scripts/blocchi.mjs <foto.bmp> [tinte] [basi] [passo] [dMin] [dMax] [incl] [nettoMm] [sormNetto] [sormSfumato] [macchiaMin] [verso]
...
import { lisciaRegione } from '../../pittorico/src/region.ts';
...
for (let t = 0; t < colori.length; t++) {
  ...
  for (const m of macchie) {
    ...
    if (process.env.DEBUG_MACCHIA && t === 4 && !debugFatto && m.areaMm2 > 15000) {
      ...
      writeFileSync('apps/pettine_v2/scripts/out/debug-macchia.svg', `<svg xmlns="http://www.w3.org/2000/svg" viewBox="${(mnx - 2).toFixed(1)} ${(mny - 2).toFixed(1)} ${(mxx - mnx + 4).toFixed(1)} ${(mxy - mny + 4).toFixed(1)}" width="1400"><rect x="${(mnx - 2).toFixed(1)}" y="${(mny - 2).toFixed(1)}" width="${(mxx - mnx + 4).toFixed(1)}" height="${(mxy - mny + 4).toFixed(1)}" fill="#fff"/>${d.join('')}</svg>`);
...
writeFileSync(`apps/pettine_v2/scripts/out/${nome}.svg`, `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${WM.toFixed(1)} ${HM.toFixed(1)}" width="${WM.toFixed(1)}mm" height="${HM.toFixed(1)}mm">
