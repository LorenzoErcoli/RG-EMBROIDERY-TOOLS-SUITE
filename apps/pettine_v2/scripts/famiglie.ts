...
//
//   npx esbuild apps/pettine_v2/scripts/famiglie.ts --bundle --format=esm --platform=node \
//     --alias:@rg/core=./packages/core/src/index.ts --outfile=apps/pettine_v2/scripts/famiglie.mjs
//   VERIFICA=1 node --max-old-space-size=6144 apps/pettine_v2/scripts/famiglie.mjs <file.svg> [basi] [sormonto] [lisciaMm]
...
writeFileSync('apps/pettine_v2/scripts/out/verifica-famiglie.svg', `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -18 ${WM.toFixed(1)} ${(HM + 18).toFixed(1)}" width="${WM.toFixed(1)}mm" height="${(HM + 18).toFixed(1)}mm">
...
console.log('-> apps/pettine_v2/scripts/out/verifica-famiglie.svg');
