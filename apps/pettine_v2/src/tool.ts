...
export function mountPettine(root: HTMLElement, opts: { backHref?: string } = {}): void {
  root.innerHTML = `
  ${topbar('Pettine V2 · sequenza locale', opts.backHref)}
...
<label class="rg-field rg-param-grid__wide">
            ...
            <span class="rg-field__help">Lunghezza reale massima del corridoio cercato sul filo futuro. La V2 non autorizza scorciatoie a vista. Le impunture di andata e ritorno seguono la base della riga che verrà ricamata.</span>
          </label>
          <label class="rg-field rg-param-grid__wide">
  ...
  const progetto = (): Record<string, unknown> => ({
    rgProject: 'pettine',
    motore: 'pettine_v2-sperimentale',
    versione: 1,
    ...
    setTimeout(() => {
      try {
        ...
        if (s.punti) righe.push(
          ...
          ['Passaggi senza copertura verificata', `${n1(s.passaggiScopertiM * 1000)} mm`],
          ['Corridoi', `${n1(s.corridoiM)} m di strada distinta · il più lungo ${n0(s.passaggioPiuLungoMm)} mm`],
          ['Riprese di riga', n0(s.righeInglobate)],
          ['Righe fuori ordine', n0(s.righeFuoriOrdine)],
        );
        if (esito.verifica) righe.push(['Punti sotto 1 mm', n0(esito.verifica.puntiSotto1Mm)], ['Verifica V2', esito.verifica.prontoPerSwatch ? 'Superata nel modello geometrico' : 'Da correggere: DST non disponibile']);
        ...
        $('status').textContent = esito.verifica && !esito.verifica.prontoPerSwatch
          ? 'Prova V2: sequenza da correggere. Il DST non è disponibile; guarda le misure di verifica.'
          : s.punti
