...
/** Opzioni del motore di percorso (tour). */
export interface TourOpts { rowMm?: number; jitter?: number; seed?: number; }

/**
 * MOTORE DI PERCORSO (STC — Spanning-Tree Covering): copre l'area con UN filo CONTINUO che non vaga a
 * caso ma percorre un "giro" sull'albero di copertura delle celle → tocca ogni zona senza infilarsi nei
 * vicoli ciechi. Si spezza SOLO tra componenti davvero separate (pochissimi stacchi). Sopra ci mette un
 * TREMOLIO casuale (jitter) per l'aspetto organico. Ritorna una lista di tratti (uno per componente).
 */
export function generateTour(boundary: Polyline, voids: Polyline[], p: InterlaceParams, opts: TourOpts = {}): Point[][] {
  ...
  const row = Math.max(0.35, opts.rowMm ?? Math.max(0.4, (p.densitySpacingMm || 1) * 0.6));
  const jitter = opts.jitter ?? 1.3; // frazione di `row` (alto = più organico, meno "pettinato")
  const seed = (opts.seed ?? p.seed ?? 1) >>> 0;

  const fineRes = Math.max(0.3, Math.min(0.5, minS / 8));
  const mask = buildMask(boundary, voids, fineRes);
  const SAFE = 0.6 * mask.res;
  const inRegion = (x: number, y: number): boolean => mask.sdf(x, y) >= clear + SAFE;
  const segStep = mask.res * 0.9;
  const segOk = (ax: number, ay: number, bx: number, by: number): boolean => {
    const L = Math.hypot(bx - ax, by - ay), n = Math.max(1, Math.ceil(L / segStep));
    for (let k = 0; k <= n; k++) { const t = k / n; if (!inRegion(ax + (bx - ax) * t, ay + (by - ay) * t)) return false; }
    return true;
  };
  const bb = bounds(boundary);
  const GW = Math.ceil((bb.maxX - bb.minX) / row) + 1, GH = Math.ceil((bb.maxY - bb.minY) / row) + 1;
  const cX = (i: number) => bb.minX + (i + 0.5) * row, cY = (j: number) => bb.minY + (j + 0.5) * row;
  const fill = new Uint8Array(GW * GH);
  for (let j = 0; j < GH; j++) for (let i = 0; i < GW; i++) if (inRegion(cX(i), cY(j))) fill[j * GW + i] = 1;

  const rng = mulberry32(seed);
  const visited = new Uint8Array(GW * GH);
  const runs: Point[][] = [];
  const neighbors = (n: number): number[] => {
    const i = n % GW, j = (n / GW) | 0, out: number[] = [];
    const cand: [number, number][] = [[i - 1, j], [i + 1, j], [i, j - 1], [i, j + 1]];
    for (const [ni, nj] of cand) {
      if (ni < 0 || nj < 0 || ni >= GW || nj >= GH) continue;
      const m = nj * GW + ni;
      if (!fill[m]) continue;
      if (!segOk(cX(i), cY(j), cX(ni), cY(nj))) continue; // vicini "separati" da un vuoto sottile → non collegati
      out.push(m);
    }
    return out;
  };
  ...
  for (let start = 0; start < GW * GH; start++) {
    ...
    // Albero di copertura (BFS) della componente, con ordine dei vicini casuale (varietà organica).
    const parent = new Int32Array(GW * GH).fill(-2);
    const order: number[] = [start]; parent[start] = -1; visited[start] = 1;
    for (let qi = 0; qi < order.length; qi++) {
      const n = order[qi], nb = neighbors(n);
      for (let s = nb.length - 1; s > 0; s--) { const r = (rng() * (s + 1)) | 0; const t = nb[s]; nb[s] = nb[r]; nb[r] = t; }
      for (const m of nb) if (!visited[m]) { visited[m] = 1; parent[m] = n; order.push(m); }
    }
    const children = new Map<number, number[]>();
    for (const n of order) children.set(n, []);
    for (const n of order) if (parent[n] >= 0) children.get(parent[n])!.push(n);
    // Euler tour dell'albero: sequenza di celle adiacenti (ogni ramo percorso due volte) → continuo.
    const seq: number[] = [start];
    const stack: [number, number][] = [[start, 0]];
    while (stack.length) {
      const top = stack[stack.length - 1], ch = children.get(top[0])!;
      if (top[1] < ch.length) { const c = ch[top[1]++]; seq.push(c); stack.push([c, 0]); }
      else { stack.pop(); if (stack.length) seq.push(stack[stack.length - 1][0]); }
    }
    if (seq.length < 2) continue;
    // 1) Percorso FITTO (jitter): tremolio su ogni cella. `row` = spaziatura delle righe (copertura),
    //    INDIPENDENTE dalla lunghezza del punto.
    const jpt = (n: number): Point => {
      const i = n % GW, j = (n / GW) | 0, px = cX(i), py = cY(j);
      for (let t = 0; t < 5; t++) { const a = rng() * 2 * Math.PI, r = rng() * jitter * row; const jx = px + Math.cos(a) * r, jy = py + Math.sin(a) * r; if (inRegion(jx, jy)) return { x: jx, y: jy }; }
      return { x: px, y: py };
    };
    const raw: Point[] = [jpt(seq[0])];
    for (let s = 1; s < seq.length; s++) {
      const a = raw[raw.length - 1];
      let b = jpt(seq[s]);
      if (!segOk(a.x, a.y, b.x, b.y)) { b = { x: cX(seq[s] % GW), y: cY((seq[s] / GW) | 0) }; if (!segOk(a.x, a.y, b.x, b.y)) continue; }
      raw.push(b);
    }
    // 2) Campiono il percorso a punti di lunghezza [min,max] (il minimo NON scende mai): copertura fitta,
    //    ma passi giusti per la macchina.
    const run: Point[] = [raw[0]];
    let acc = 0, target = minS + rng() * (maxS - minS);
    for (let s = 1; s < raw.length; s++) {
      let ax = raw[s - 1].x, ay = raw[s - 1].y; const bx = raw[s].x, by = raw[s].y;
      let segLen = Math.hypot(bx - ax, by - ay);
      while (acc + segLen >= target && segLen > 1e-9) {
        const t = (target - acc) / segLen;
        ax = ax + (bx - ax) * t; ay = ay + (by - ay) * t;
        run.push({ x: ax, y: ay });
        segLen = Math.hypot(bx - ax, by - ay);
        acc = 0; target = minS + rng() * (maxS - minS);
      }
      acc += segLen;
    }
    // 3) Pulizia punto minimo (i ritorni sulle foglie dell'Euler tour creano micro-segmenti): tengo un
    //    punto solo se dista ≥ minS dal precedente tenuto (l'ultimo si tiene sempre). Il minimo non scende.
    if (run.length >= 2) {
      const clean: Point[] = [run[0]];
      for (let s = 1; s < run.length; s++) {
        const l = clean[clean.length - 1];
        const d = Math.hypot(run[s].x - l.x, run[s].y - l.y);
        if (d < minS - 0.001 && s < run.length - 1) continue; // micro-segmento → salta (l'ultimo si tiene)
        if (d > maxS) { const k = Math.ceil(d / maxS); for (let q = 1; q <= k; q++) clean.push({ x: l.x + (run[s].x - l.x) * q / k, y: l.y + (run[s].y - l.y) * q / k }); }
        else clean.push(run[s]);
      }
