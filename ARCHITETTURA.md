...
graph TD
  ...
  subgraph Packages["packages/ — CODICE CONDIVISO"]
    CORE["core<br/>units · io(svg/dxf) · geometry · stitch · fill(R24)<br/>quantize · underlay · routing<br/>covered-travel · tension · export"]
...
5. Il satellite Python **non entra nel workspace-link**: condivide contratti (`params.schema.json`), non codice. Gli **strumenti di sviluppo** in Python che vivono nel repo stanno in `strumenti-sviluppo/<nome>`, fuori da `apps/*`: si lanciano a mano (`npm run sviluppo:<nome>`), la suite non li esegue e la CI non li installa.
...
**Capacità globali attive** (una volta, per tutti — vedi Costituzione): export **SVG e DST riapribili** (R9/R27/R31 — `readProjectMetadata` per l'SVG, `readDstMetadata` per il footer del DST; la cucitura resta byte-identica) — **vero per tutti e otto i tool dal 2026-09-03, e verificato tool per tool da uno smoke test**: prima era dichiarato qui ma implementato solo in due, ed è il motivo per cui ora è un test e non una frase, export **DST macchina** (R31, `dstFromExportLayers`), **salvataggio con finestra di sistema** (R29, in una fase con `saveTextFile`/`saveBinaryFile` o in **due** con `pickSaveTarget` — destinazione prima, calcolo dopo: serve a chi ha un export lento, perché la finestra la concede solo l'attivazione fresca del clic), **pannello canonico** Testa A/B + accordion (DS v1.7.0), **passaggi che girano attorno alle aree vuote** (R5, `avoidVoids` in `travel.ts`), **cattura colore da immagine** (`quantize.ts`: median-cut deterministico, colore più vicino — estratta quando l’ha chiesta il terzo tool), **riempimento a righe parallele / raso** (`fill.ts`, R24 — pettine o serpentina, dentro un poligono coi fori: era «il grande assente»), **semplificazione dei contorni** (`simplifyPolyline`, promossa da oblique), **dalla maschera di colore alle regioni coi loro fori** (`regions.ts`, `traceRegions`) e **dall'immagine alle tinte stabili** (`reduce.ts`: pareggio della luce, attenuazione della grana, palette affinata, isole assorbite) — tutte e due promosse da broccato quando le ha chieste il Punto Pittorico, col lucchetto scritto *prima* del trasloco, **i passaggi nascosti** (`routing.ts`, R16–R21: mappa di ciò che verrà cucito sopra, catena minima fra le macchie, e per ogni tratto dritto → nascosto con l'A* → staccato solo se non c'è strada; il chiamante consegna le corse **già in fila**, perché l'ordine dentro una macchia lo sa solo chi ha generato il riempimento) — promosso da broccato, terzo pezzo chiesto dal Punto Pittorico, **guida in-app** generata da `MANUALE.md` (bottone *Guida* nella topbar di ogni tool).

**Rete di sicurezza comune:** `npm test` (668 asserzioni: le primitive del core più le invarianti di tutti i motori, su fixture sintetiche **e sugli SVG veri**), `npm run typecheck` (la build non controlla i tipi: vite usa esbuild) e `npm run build`. Tutti e tre girano in CI a ogni push.
