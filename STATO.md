...
- **Multicolore a stop rotanti.** Un solo filo tagliato in cambi-ago che **ruotano sulla palette** (numero variabile di colori, `paletteCycles` ripetizioni): ogni colore ricompare su più strati sovrapposti — l'ultimo **non** solo in cima. Un layer/gruppo per colore in export (come l'SVG di riferimento). **UI palette nel pannello** (sezione 03 "Colori del filo": swatch editabili col picker nativo, aggiungi/rimuovi, ripetizioni) — composta dal subagent design-system con i soli componenti v1.6.0 (`rg-color-map`, `rg-icon-button--danger`, `rg-cluster`, `rg-input--numeric`), **nessuna classe nuova, nessun commit sul DS**.
...
- **Densità più uniforme (anti-grumo).** Lo scoring preferisce la cella meno coperta, scoraggia i micro-giri nella stessa cella (fonte dei grumi) e usa meno rumore. Migliorato, ma su labirinti fitti non è ancora perfettamente omogeneo (vedi §3).
...
- [x] ~~**Nuovo tool in arrivo**~~ → è **`interlace`** (riempimento a intreccio/nuvola multicolore). Definito con Lorenzo, piano in 5 punti confermato. **Fatto lo step 1+3 base:** scaffold + motore di riempimento a passaggi brevi su sagoma demo, vuoti/bordo rispettati, export SVG. Restano gli step successivi:
  ...
  - [x] ~~**interlace — multicolore a stop rotanti**~~: motore/pipeline + **UI palette nel pannello** (sezione "Colori del filo", componenti DS v1.6.0). Fatto.
