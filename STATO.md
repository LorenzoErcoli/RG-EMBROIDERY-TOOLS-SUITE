...
- **Revisione dei parametri — 8 decisioni CHIUSE** → `REVISIONE-PARAMETRI.md` (tabella in testa). Da implementare, in quest'ordine:
  1. **Costituzione §3**: registra i nomi/le unità decisi. Chiarisci `cordDensityMm` (interlinea = passo *longitudinale* tra punti consecutivi) vs `densitySpacingMm` (spaziatura *trasversale*) — erano lo stesso nome per due grandezze. Aggiungi le rinomine ⑧ alle colonne "Sostituisce". Registra lo `strokeWidth` spezzato (filo 0,1 + spessore di costruzione) e l'onda in mm/gradi.
  2. **net-45**: etichette (①②③) + riordino pannello DS (01 Sagoma con `realWidthMm`, 02 Colori e ruoli con `rg-color-map`, 03 Parametri) + `rg-file-input` + valori in `rg-input--numeric`.
  3. **pattern-grammar**: riordino pannello (01 Sagoma · 02 Colori e ruoli · 03 Formato · 04–07 gruppi · 08 Preset) + etichette + ⑤ larghezza esatta + ⑥ split `strokeWidth` + ⑦ onda mm/gradi + ⑧ rinomina con **conversione preset**.
  4. **④** densità rasi/quadratini: restano nascoste finché non ci sono i riempimenti veri.
  5. Aggiorna `test/smoke.mjs` dove le conversioni (preset, onda) lo permettono.
