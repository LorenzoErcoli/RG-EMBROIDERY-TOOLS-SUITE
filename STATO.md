...
- **Multicolore MÉLANGE (no macchie, densità esatta).** *Evoluzione:* (1) colori a rotazione lungo la sequenza → macchie; (2) N passate separate col dither → melange ma **filo raddoppiato** (sfori); (3) **attuale:** UN SOLO riempimento **uniforme** (densità esatta, CoV basso) i cui segmenti sono **colorati con un dither per cella** (ogni cella un colore in base alla posizione, grana ~ lunghezza punto). Melange fine su tutta l'area, **densità invariata** (sul file reale denso: 347m → 50m). `paletteCycles` = n. ripetizioni palette (n. stop = colori × cicli; default 2). **UI palette nel pannello** (sezione 03 "Colori del filo": swatch editabili col picker nativo, aggiungi/rimuovi, ripetizioni) — composta dal subagent design-system con i soli componenti v1.6.0 (`rg-color-map`, `rg-icon-button--danger`, `rg-cluster`, `rg-input--numeric`), **nessuna classe nuova, nessun commit sul DS**.
...
- [x] ~~**Nuovo tool in arrivo**~~ → è **`interlace`** (riempimento a intreccio/nuvola multicolore). Definito con Lorenzo, piano in 5 punti confermato. **Fatto lo step 1+3 base:** scaffold + motore di riempimento a passaggi brevi su sagoma demo, vuoti/bordo rispettati, export SVG. Restano gli step successivi:
  ...
  - [ ] **interlace — residui di disomogeneità**: restano un anello perimetrale più rado e qualche riga più carica nei canali dritti (CoV ~0.67 sui parametri densi). Migliorabile se serve.
