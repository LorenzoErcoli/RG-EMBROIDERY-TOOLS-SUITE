...
- **Densità = dimensione della cella (spaziatura fra le file) — RISCRITTA.** *Prima* `densitySpacingMm` guidava il **target di copertura** per cella (`T=round(cell/spacing)`) con cella FISSA a ~2mm → la densità aveva **solo 2 livelli** (1 o 2) e, peggio, a densità bassa il filo AUMENTAVA (i tragitti dominavano: densità 3 dava più filo di 1). *Ora* la densità **È la dimensione della cella** (spaziatura fra le file, ~0.8–3.2mm) con target FISSO: cella piccola = fitto/più filo, cella grande = rado/meno filo, con **gamma continua e monotòna** nel range utile. Verificato su file FF (1 colore): densità 1→83m, 1.5→46m, 2→30m, 3→21m. Da 3 a **6mm** (tetto) il filo si fa più **rado** (buchi voluti, si perde copertura) ma in modo irregolare (aliasing cella/geometria); a densità 5 il look è ben sparso. Oltre ~6-7 il filo continuo **esplode** in tragitti/autostrade (più filo, non più rado) → tetto a 6.
