...
- **Multicolore MÉLANGE (no macchie).** *Prima versione:* colori a rotazione lungo la sequenza del filo → ogni colore restava in una zona → **macchie** (segnalato da Lorenzo con screenshot Stilista). *Ora:* la copertura totale (densità giusta) è **distribuita tra le passate/colori con un dither spaziale** — ogni cella riceve le sue attraversate da colori diversi a seconda della posizione. Risultato: ogni colore è **sparso su tutta l'area**, il colore si **mescola ovunque**, densità invariata. `paletteCycles` = quante volte gira la palette (n. stop = colori × cicli; default 2). Motore rifattorizzato: `prepare` (maschera, una volta) + `runOneFill` (una passata, target di copertura **per-cella**) + `generatePasses` (N passate col dither). **UI palette nel pannello** (sezione 03 "Colori del filo": swatch editabili col picker nativo, aggiungi/rimuovi, ripetizioni) — composta dal subagent design-system con i soli componenti v1.6.0 (`rg-color-map`, `rg-icon-button--danger`, `rg-cluster`, `rg-input--numeric`), **nessuna classe nuova, nessun commit sul DS**.
...
- [x] ~~**Nuovo tool in arrivo**~~ → è **`interlace`** (riempimento a intreccio/nuvola multicolore). Definito con Lorenzo, piano in 5 punti confermato. **Fatto lo step 1+3 base:** scaffold + motore di riempimento a passaggi brevi su sagoma demo, vuoti/bordo rispettati, export SVG. Restano gli step successivi:
  ...
  - [ ] **interlace — inflazione filo nel mélange**: le passate depositano ~2× il filo teorico (il filo attraversa celle non assegnate a quel colore → bleed). Visivamente ok, ma la densità reale supera quella impostata. Da limare (es. penalità più dura sulle celle non assegnate, o densità corretta a valle).
