...
- **Multicolore a stop rotanti.** Un solo filo tagliato in cambi-ago che **ruotano sulla palette** (numero variabile di colori, `paletteCycles` ripetizioni): ogni colore ricompare su più strati sovrapposti — l'ultimo **non** solo in cima. **UI palette nel pannello** (sezione 03 "Colori del filo": swatch editabili col picker nativo, aggiungi/rimuovi, ripetizioni) — composta dal subagent design-system con i soli componenti v1.6.0 (`rg-color-map`, `rg-icon-button--danger`, `rg-cluster`, `rg-input--numeric`), **nessuna classe nuova, nessun commit sul DS**.
