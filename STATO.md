...
- **Multicolore = PASSATE sovrapposte, con MOTORE DI PERCORSO (tour).** Ogni colore è **un filo continuo** che copre tutta la superficie; i colori (semi diversi) si sovrappongono → intreccio. È il modello del riferimento (2 grigi), esteso a più colori. `densitySpacingMm` = densità di ogni colore; totale = × n. passate (colori × cicli). Il fill NON è più un vagare casuale (in labirinto: grumi + tanti stacchi) ma un **giro sull'albero di copertura** (`generateTour`, STC): **filo continuo con pochissimi stacchi** (solo tra zone separate: sul file FF **1 stacco/colore**, come il riferimento ~3), **copertura piena** (0% vuoti), **uniforme** (CoV ~0.5 = riferimento), punto min mai violato / max sottodiviso, bordi/vuoti rispettati. Sopra c'è un **tremolio** (`jitter`, interno) per l'organico. **NODO APERTO = il carattere:** il tour tende al "pettinato"; col tremolio alto si avvicina al casuale ma resta un filo di direzionalità → **da giudicare a occhio da Lorenzo** in `avvia.bat` (il `jitter` è la manopola). *(Scartati perché snaturavano l'effetto o non tenevano i vincoli: vagare casuale = grumi+80 stacchi; continuo puro = 282m; dither per-cella = mille cambi-colore; rotazione-sequenza = macchie.)* **UI palette nel pannello** (sezione 03 "Colori del filo": swatch editabili col picker nativo, aggiungi/rimuovi, ripetizioni) — composta dal subagent design-system con i soli componenti v1.6.0 (`rg-color-map`, `rg-icon-button--danger`, `rg-cluster`, `rg-input--numeric`), **nessuna classe nuova, nessun commit sul DS**.
