...
- **Multicolore = PASSATE sovrapposte** (modello confermato con Lorenzo). Ogni colore è **un filo continuo che percorre tutta la superficie** (passata uniforme, seme diverso); i colori si sovrappongono → intreccio. È il modello del suo SVG di riferimento (2 grigi), esteso a più colori. `densitySpacingMm` = densità **di ogni colore**; totale = quella × n. passate (colori × cicli). Default: 4 colori, 1 ciclo, densità 1.0. *(Tentativi scartati perché snaturavano l'effetto: rotazione-lungo-sequenza = macchie; dither per-cella = mille cambi-colore.)* **UI palette nel pannello** (sezione 03 "Colori del filo": swatch editabili col picker nativo, aggiungi/rimuovi, ripetizioni) — composta dal subagent design-system con i soli componenti v1.6.0 (`rg-color-map`, `rg-icon-button--danger`, `rg-cluster`, `rg-input--numeric`), **nessuna classe nuova, nessun commit sul DS**.
...
- **Labirinto: tratti multipli con salti a penna alzata.** L'area ricamabile è spesso un labirinto (canali che girano attorno agli ostacoli); un solo filo continuo resterebbe intrappolato in una tasca. Il motore, quando una tasca è piena o bloccata, **si riloca** alla zona scoperta più vicina aprendo un **nuovo tratto** (il salto NON è disegnato → non attraversa il vuoto). `generateFill` ritorna quindi una **lista di tratti**; i colori ruotano su tutta la sequenza senza unire linee tra un tratto e l'altro.
...
- [x] ~~**Nuovo tool in arrivo**~~ → è **`interlace`** (riempimento a intreccio/nuvola multicolore). Definito con Lorenzo, piano in 5 punti confermato. **Fatto lo step 1+3 base:** scaffold + motore di riempimento a passaggi brevi su sagoma demo, vuoti/bordo rispettati, export SVG. Restano gli step successivi:
  ...
  - [ ] **interlace — uniformità nei labirinti stretti (limite noto, DECISIONE aperta)**: su forme aperte il riempimento è già uniforme (CoV 0.36, meglio del riferimento 0.52); su un cartamodello a **canali stretti** (es. lettere FF) una passata è irregolare (~1.0), 4 colori sovrapposti ~0.72. Il caso peggiore del riempimento casuale sono i canali stretti e vincolanti. Opzioni da valutare con Lorenzo: (A) usarlo su sagome aperte dove è già ottimo; (B) per i labirinti, aggiungere un riempimento **strutturato che segue i canali** (serpentina/tatami con jitter organico) — evade il caso peggiore ma cambia un po' il carattere; (C) accettare ~0.7 con più colori / densità moderata. Fatti finora: bordi pieni (0% buchi), niente più sfori/bleed, guida-lungo-canale nei tratti stretti.
