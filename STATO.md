...
- **Multicolore a stop rotanti.** Un solo filo tagliato in cambi-ago che **ruotano sulla palette** (numero variabile di colori, `paletteCycles` ripetizioni): ogni colore ricompare su più strati sovrapposti — l'ultimo **non** solo in cima. Un layer/gruppo per colore in export (come l'SVG di riferimento). Motore/pipeline fatti e verificati; **manca la UI della palette nel pannello** (in arrivo dal subagent design-system).
...
- [x] ~~**Nuovo tool in arrivo**~~ → è **`interlace`** (riempimento a intreccio/nuvola multicolore). Definito con Lorenzo, piano in 5 punti confermato. **Fatto lo step 1+3 base:** scaffold + motore di riempimento a passaggi brevi su sagoma demo, vuoti/bordo rispettati, export SVG. Restano gli step successivi:
  ...
  - [~] **interlace — multicolore a stop rotanti**: motore/pipeline **fatti** (filo tagliato in cambi-ago che ruotano sulla palette; un layer per colore). Resta la **UI palette nel pannello** (numero colori variabile + swatch editabili + ripetizioni), in composizione via subagent design-system con i componenti v1.6.0.
