...
- **Revisione dei parametri — 8 decisioni CHIUSE** → `REVISIONE-PARAMETRI.md` (tabella in testa). Implementazione in corso:
  ...
  3. [ ] **pattern-grammar** (il grosso): riordino pannello + etichette + ⑤ larghezza esatta + ⑥ split `strokeWidth` (filo 0,1 + `constructionStrokeMm`) + ⑦ onda mm/gradi + ⑧ rinomina con **conversione preset**. **In attesa** della riflessione DS sotto (l'ordine del pannello potrebbe cambiare).
...
- **Riflessione DS aperta (in corso, agente `design-system`)**: due direttive di Lorenzo sulla struttura del pannello — (1) **Formato in cima**, poi Sagoma+colori sotto (inverte l'ordine canonico attuale; da riconciliare tra tool con e senza formato proprio); (2) **sezioni tutte in accordion** apribili/chiudibili (regola su default aperto/chiuso, persistenza, soglia). pattern-grammar aspetta questa risposta prima del riordino, così non si rifà due volte.
