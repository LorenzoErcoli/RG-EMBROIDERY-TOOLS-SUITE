...
- **`ds/workspace-panel-density` in attesa di merge**: corregge due difetti del DS emersi dal pannello troppo stretto (token `--rg-layout-tool-panel`, e `rg-param-grid` che impila via *container query* sulla larghezza del pannello invece che della finestra, più `min-width: 0` sull'input con unità). Sta su `0bd0905`, pushato, **non mergiato**. Il fix lato app funziona già su v1.2.0; col merge arriva la rete di sicurezza automatica. Vedi sopra: ora è una di tre proposte concorrenti.
...
- **Struttura del pannello laterale — direttiva ricevuta, adozione BLOCCATA.** L'agente `design-system` ha definito l'ordine canonico (testa: Sagoma · Colori e ruoli · Formato e scala → corpo: gruppi del tool nel loro ordine → coda: Esportazione · Preset) con un *test di appartenenza* che segue il verso della lavorazione, non l'argomento. Sta su `ds/tool-panel` (`431d61f`, pushato, **non mergiato**), con due componenti nuovi (`rg-file-input`, `rg-color-map`) e la composizione corretta della sezione richiudibile. **Non adottabile finché il submodule è a v1.2.0**: quelle classi lì non esistono.
  ...
  - *Ordini risultanti da implementare*: **net-45** → 01 Sagoma (file + `realWidthMm`, che oggi sta fra i parametri + Sagoma demo) · 02 Colori e ruoli · 03 Parametri. **pattern-grammar** → 01 Sagoma · 02 Colori e ruoli · 03 Formato e scala · 04–07 i gruppi attuali · 08 Preset.
- **Tre proposte DS concorrenti, tutte a 1.4.0, tutte da mergiare da Lorenzo.** `ds/tool-panel` (pannello), `ds/config-panel` (`rg-input--numeric` per i valori in mm), `ds/workspace-panel-density` (token e container query). Sono **complementari, non alternative**: chi entra per secondo rinumera la versione e riconcilia `components.json` e `patterns/workspace.md`. Finché non c'è un tag, il submodule resta a v1.2.0 e la suite non può adottare niente di tutto questo.
